...
import { normalizeOption, normalizeProduct } from '../../src/utils/menuNormalizer.js';
import { getOptionsPriceByRule, normalizeCartItem, useCartStore } from '../../src/store/cart.js';
...
test('menuNormalizer cobre sum/highest/average + aliases single/multiple', () => {
  ...
  const product = normalizeProduct({
    id: 1,
    name: 'Pizza',
    options: [{ id: 10, type: 'single', items: [{ id: 101, name: 'Cheddar', is_active: true }] }],
  });
...
test('serializer gera items[].options[] flat com option_* e item_*', () => {
  ...
  store.addItem(
    { id: 77, name: 'Pizza G', base_price: 59.9 },
